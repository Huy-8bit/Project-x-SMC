...
contract BaseXNFT is ERC721URIStorage, Ownable {
    ...
    NFT[] private mintedNFTs; // An array to store information about all minted NFTs
    ...
    function mintNFT(string memory _NFT_url) public payable {
        ...
        NFT memory newNFT = NFT(newTokenId, randomRank);
        mintedNFTs.push(newNFT); // Add the new minted NFT to the array
    ...
    function getNFTRank(uint256 tokenId) public view returns (Rank) {
        require(_exists(tokenId), "Token ID does not exist");
        return mintedNFTs[tokenId - 1].rank; // Adjust the index as the array is 0-based
    }
